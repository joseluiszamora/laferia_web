...
export async function toggleTiendaStatus(id: string) {
  try {
    // Get the current tienda to check its status
    const currentTienda = await prisma.store.findUnique({
      where: { storeId: parseInt(id) },
      select: { status: true },
    });
    ...
    const newStatus = currentTienda.status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
